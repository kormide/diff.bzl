#!/usr/bin/env bash
#
# Shows a sample end-to-end workflow for accepting new patches without failing the build.

set -o errexit -o pipefail -o nounset

if [ "$#" -eq 0 ]; then
	echo "usage: build-and-patch.sh [target pattern...]"
	exit 1
fi
...
args=(
	...
	"--@diff.bzl//diff:validate"
)

# Run build with validation enabled and output group for patches.
